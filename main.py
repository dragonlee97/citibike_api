...
@api.get("/user_demographics", response_model=UserDemographicsResponse)
def user_demographics(client: bigquery.Client = Depends(connect_to_bq)):
...
@api.post("/station_traffic", response_model=StationReponse)
def station_traffic(station_request: StationRequest = Body(..., embed=True), client: bigquery.Client = Depends(connect_to_bq)):
    with open("sql/station_traffic.sql") as file:
        sql_template = file.read()
    template = Template(sql_template)
    sql_query = template.render(station_request=station_request, project=os.getenv("PROJECT"), dataset=os.getenv("DATASET"))
    try:
        query_job = client.query(sql_query)
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"sql query failed: {e}")
    return {
        "request_id": station_request.request_id,
